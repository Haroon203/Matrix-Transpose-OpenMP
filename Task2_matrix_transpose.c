...
void transposeParallel(int matrix[N][N], int transposed[N][N], int use_dynamic)
{
    ...
    {
#pragma omp parallel for schedule(dynamic) collapse(2) // Parallel loop using OpenMP with static scheduling
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < N; j++)
            {
#pragma omp critical // Critical section ensures only one thread modifies at a time
    ...
    else // If dynamic static is selected
    {
#pragma omp parallel for schedule(static) collapse(2)
        for (int i = 0; i < N; i++)
        {
            ...
            {
                int temp = matrix[i][j];
#pragma omp atomic write // Atomic write ensures only one thread writes at a time
                transposed[j][i] = temp;
            }
        }
    }
